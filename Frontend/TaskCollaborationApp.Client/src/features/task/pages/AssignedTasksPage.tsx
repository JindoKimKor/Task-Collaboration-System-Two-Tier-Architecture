...
import { fetchAssignedTasks } from "../store/taskThunks";
 ...
 *
 * Client 활용:
 * - /tasks/assigned 경로에서 렌더링
 * - 현재 로그인 유저가 AssignedTo인 태스크만 표시
 *
 * Container Component 역할:
 * - Redux 연결 (useAppDispatch, useAppSelector)
 * - 데이터 fetch (fetchAssignedTasks)
...
export const AssignedTasksPage = () => {
  ...
  useEffect(() => {
    dispatch(fetchAssignedTasks({ pageSize: 100 }));
  }, [dispatch]);
